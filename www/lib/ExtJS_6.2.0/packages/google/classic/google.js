Ext.define("Ext.google.ux.Client", {
    ...
    requiredScripts: ["//apis.google.com/js/client.js?onload=_ext_google_ux_client_initialize_"],
    statics: {
        getApiVersion: function (b) {
            var a = this.libraries[b];
            return a && a.state == 2 ? a.version : null
        }
    },
    mixinConfig: {
        extended: function (c, a, b) {
            this.load(b.googleApis)
        }
    },
    onClassMixedIn: function (a) {
        this.load(a.prototype.googleApis)
    },
    privates: {
        statics: {
            initialized: false, blocked: false, loading: 0, libraries: {}, load: function (d) {
                var c = this.libraries, a, b;
                if (!Ext.isObject(d)) {
                    return
                }
                Ext.Object.each(d, function (f, e) {
                    a = e.version || "v1";
                    b = c[f];
                    if (!Ext.isDefined(b)) {
                        c[f] = {version: a, state: 0}
                    } else {
                        if (b.version !== a) {
                            Ext.log.error('Google API: failed to load version "' + a + '" of the', '"' + f + '" API: "' + b.version + '" already loaded.')
                        }
                    }
                });
                this.refresh()
                ...
                Ext.Object.each(a.libraries, function (c, b) {
                    if (b.state == 0) {
                        b.state = 1;
                        gapi.client.load(c, b.version, function () {
                            b.state = 2;
                            if (!--a.loading) {
                                a.refresh()
                            }
...
Ext.define("Ext.google.data.AbstractProxy", {
    ...
    batchActions: false,
    reader: {type: "json", rootProperty: "items", messageProperty: "error"},
    doRequest: function (a) {
        var d = this, c = d.buildRequest(a), e = d.getWriter(), b = false;
        if (e && a.allowWrite()) {
            c = e.write(c)
        }
        d.execute(d.buildApiRequests(c)).then(function (f) {
            d.processApiResponse(a, c, f)
        });
        return c
    },
    buildUrl: function (a) {
    ...
    privates: {
        execute: function (b) {
            b = [].concat(b);
            var a = [];
            return Ext.Array.reduce(b, function (d, c) {
        ...
        }, processApiResponse: function (a, d, e) {
            var b = false, c = [];
            Ext.each(Object.keys(e.result), function (g) {
                var f = e.result[g].result;
                if (f.error) {
                    b = f.error.message;
                    return false
                }
            ...
            this.processResponse(true, a, d, {results: b ? [] : c, success: !b, error: b})
        }, sanitizeItems: function (a) {
            var b = [], c = [];
            Ext.Array.each(a, function (d) {
                if (!Ext.Array.contains(c, d.id)) {
                    b.push(d);
                    c.push(d.id)
                }
            }, this, true);
...
Ext.define("Ext.google.data.EventsProxy", {
    ...
    buildApiRequests: function (b) {
        var a = this, c = b.getAction();
        switch (c) {
            case"read":
                return a.buildReadApiRequests(b);
            case"create":
                return a.buildCreateApiRequests(b);
            case"update":
                return a.buildUpdateApiRequests(b);
            case"destroy":
                return a.buildDestroyApiRequests(b);
            default:
                Ext.raise("unsupported request: events." + c);
                return null
        }
    },
    extractResponseData: function (b) {
        var c = this, d = c.callParent(arguments), a = [];
        Ext.each(d.results, function (e) {
            switch (e.kind) {
                case"calendar#events":
                    a = a.concat(e.items.map(c.fromApiEvent.bind(c)));
                    break;
                case"calendar#event":
                    a.push(c.fromApiEvent(e));
                    break;
                default:
                    break
            }
        });
        return {items: c.sanitizeItems(a), success: d.success, error: d.error}
    },
    privates: {
        toApiEvent: function (c, b) {
            var a = {};
                ...
                switch (e) {
                    ...
                    case"endDate":
                        if (b) {
                            ...
                            f = Ext.Date.format(new Date(g), "c")
                        }
                        a[e.slice(0, -4)] = {date: d, dateTime: f};
                        break;
                    default:
                        break
                }
            ...
            return a
        }, fromApiEvent: function (b) {
            var a = {allDay: true};
            Ext.Object.each(b, function (d, f) {
                var c, g, e;
                switch (d) {
                    case"id":
                    case"description":
                    ...
                    case"start":
                    case"end":
                        c = Ext.Date.parse(f.dateTime || f.date, "C");
            ...
            return a
        }, buildReadApiRequests: function (d) {
            var b = d.getParams(), f = new Date(b.startDate), a = new Date(b.endDate), e = [], c;
            while (f < a) {
                c = Ext.Date.add(f, Ext.Date.MONTH, 3);
                if (c > a) {
                    c = a
                }
                e.push(gapi.client.calendar.events.list({
                    calendarId: b.calendar,
                    timeMin: Ext.Date.format(f, "C"),
                    timeMax: Ext.Date.format(c, "C"),
                    singleEvents: true,
                    maxResults: 2500
                }));
                f = c
            ...
            return e
        }, buildCreateApiRequests: function (b) {
            var a = b.getRecords()[0];
            return gapi.client.calendar.events.insert(this.toApiEvent(b.getJsonData(), a.get("allDay")))
        }, buildUpdateApiRequests: function (d) {
            var a = d.getRecords()[0], f = this.toApiEvent(d.getJsonData(), a.get("allDay")),
                c = a.getModified("calendarId"), e = a.get("calendarId"), b = a.getId(), g = [];
            f.calendarId = e;
            f.eventId = b;
            if (c && c !== e) {
            ...
            if (Object.keys(f).length > 2) {
                g.push(gapi.client.calendar.events.patch(f))
            }
            return g
        }, buildDestroyApiRequests: function (b) {
            var a = b.getRecords()[0];
            data = b.getJsonData();
            data.calendarId = data.calendarId || a.get("calendarId") || a.getPrevious("calendarId");
...
Ext.define("Ext.google.data.CalendarsProxy", {
    ...
    buildApiRequests: function (b) {
        ...
        switch (c) {
            ...
            case"update":
                return a.buildUpdateApiRequests(b);
            default:
                Ext.raise("unsupported request: calendars." + c);
                return null
        }
    },
    extractResponseData: function (b) {
        ...
        Ext.each(d.results, function (e) {
            switch (e.kind) {
                case"calendar#calendarList":
                    a = a.concat(e.items.map(c.fromApiCalendar.bind(c)));
                    break;
                default:
                    break
            }
    ...
    privates: {
        toApiCalendar: function (b) {
            ...
            return a
        }, fromApiCalendar: function (b) {
            var a = {
                hidden: !b.selected,
                editable: false,
                eventStore: {autoSync: true, proxy: {type: "google-events", resourceTypes: "events"}}
            };
            Ext.Object.each(b, function (c, d) {
                switch (c) {
                    case"id":
            ...
            return a
        }, buildReadApiRequests: function (a) {
            return gapi.client.calendar.calendarList.list()
        }, buildUpdateApiRequests: function (a) {
            var b = this.toApiCalendar(a.getJsonData());
            return gapi.client.calendar.calendarList.patch(b)
        }
    }
});
